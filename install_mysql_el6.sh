...
if [ ! -z "$exist_port" ]; then
        echo "error:  port $port is exist ,please alter mysql server port "
        exit 1
fi
...
install_rpm()
{
    ...
    cd ${mysql_dir} 
    rpm2cpio mysql-community-common-5.7.27-1.el6.x86_64.rpm  | cpio -idvm
    rpm2cpio mysql-community-libs-5.7.27-1.el6.x86_64.rpm  | cpio -idvm
    rpm2cpio mysql-community-client-5.7.27-1.el6.x86_64.rpm  | cpio -idvm
    rpm2cpio mysql-community-server-5.7.27-1.el6.x86_64.rpm  | cpio -idvm
...
init_mysql()
{
    ...
    $mysql_dir/usr/bin/mysqld_safe --defaults-file=$mysql_dir/etc/my.cnf --user=root >/dev/null  2>&1 &
